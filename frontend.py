...
def find_nearest_edge(point_lat, point_lon, stations, edges):
    ...
    for edge in edges:
        ...
        if not source or not target:
            continue
        if edge.get("is_transfer") or source.get("line_id") != target.get("line_id"):
            continue
    ...
    if not best:
        return None, None
    return best, edge_line_id(best, station_by_id)
...
def draw_network(m, stations, edges, lines, selected_line_id=None, selected_edge_id=None, show_station_dots=False):
    ...
    for edge in edges:
        ...
        is_selected_line = selected_line_id and line_id == selected_line_id
        ...
        color = line_color_by_id.get(line_id, "#6B7280")
        weight = 7 if is_selected_edge else (5 if is_selected_line else 1.2)
        opacity = 0.92 if is_selected_line else 0.18
        line_kwargs = {
            "locations": [
...
with left_col:
    ...
    if not st.session_state.token:
        ...
        show_station_dots = st.checkbox("Hiện chấm ga trên bản đồ", value=False, key="show_station_dots")
        ...
        if st.session_state.selected_network_line_id:
            ...
            st.caption(f"Đang làm nổi bật: {selected_line_name}")
            ...
            with st.expander("🛠️ Quản trị (Admin)"):
                ...
                if edges_list:
                    ...
                    selected_edge_label = st.selectbox(
                        "Chọn cạnh đang hoạt động cần chặn",
                    ...
                    if st.button("Chặn cạnh này", use_container_width=True, key="ban_selected_edge"):
                        ...
                        if edge_res.status_code == 200:
                            st.toast(f"Dã chặn cạnh ID {selected_edge_id}")
                            clear_map_cache()
                            st.rerun()
                        else:
